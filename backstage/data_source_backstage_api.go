...
const (
	descriptionEntitySpec              = "Spec is the specification data describing the entity itself."
	...
	descriptionEntityMetadataUID       = "UID A globally unique ID for the entity. This field can not be set by the user at creation time, and the server will reject an " +
		"attempt to do so. The field will be populated in read operations."
	descriptionEntityMetadataEtag = "Etag is an opaque string that changes for each update operation to any part of the entity, including metadata. This field can not be " +
	...
	descriptionEntityMetadataTitle       = "Title is a display name of the entity, to be presented in user interfaces instead of the name property, when available."
	descriptionEntityMetadataDescription = "Description is a short (typically relatively few words) description of the entity."
	descriptionEntityMetadataLabels      = "Labels are key/value pairs of identifying information attached to the entity."
	descriptionEntityMetadataAnnotations = "Annotations are key/value pairs of non-identifying auxiliary information attached to entity."
	descriptionEntityMetadataTags        = "Tags is a list of single-valued strings, to for example classify catalog entities in various ways."
	descriptionEntityMetadataLinks       = "Links is a list of external hyperlinks related to the entity. Links can provide additional contextual information that may be " +
	...
	descriptionEntityLinkTitle               = "Title is a user-friendly display name for the link."
	descriptionEntityLinkIco                 = "Icon is a key representing a visual icon to be displayed in the UI."
	descriptionEntityLinkType                = " Type is an optional value to categorize links into specific groups."
	...
	descriptionEntityRelationTargetRef       = "TargetRef is the entity ref of the target of this relation."
	descriptionEntityRelationTarget          = "Target is the entity of the target of this relation."
...
const (
	...
	descriptionApiSpecOwner      = "Owner is entity reference to the owner of the API"
	descriptionApiSpecDefinition = "Definition of the API, based on the format defined by the type."
	descriptionApiSpecSystem     = "System is entity reference to the system that the API belongs to."
)
...
func (d *apiDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError(
			...
			fmt.Sprintf("Could not read Backstage API kind %s/%s: %s",
				state.Namespace.ValueString(), state.Name.ValueString(), err.Error()),
		)
		return
	}
	...
	if response.StatusCode != http.StatusOK {
		resp.Diagnostics.AddError(
			"Error reading Backstage API kind",
			fmt.Sprintf("Could not read Backstage API kind %s/%s: %s",
				state.Namespace.ValueString(), state.Name.ValueString(), response.Status),
		)
		return
	}
